...
rxReadWrite = re.compile(r'[\s]*(READ|WRITE)'+openBracket+ r'(?:' + identifierReString + r'|' + numberReString + r')' +closedBracket+'(;)')
...
class Program:
	...
	def parseDeclarations(self, line):
		tokens = rxDeclaration.findall(line)[0]
		variables, declType = tokens[0], tokens[2].strip()
	...
	def parseReadWrite(self, line):
		tokens = filter(lambda x: x != '', rxReadWrite.findall(line)[0])
		readOrWrite, oBracket, identifier, cBracket, semiCol = tokens
		# print readOrWrite, oBracket, identifier, cBracket, semiCol
		self.PIF.append(Atom(self.atoms[readOrWrite.upper()], '-', readOrWrite))
		self.PIF.append(Atom(self.atoms[oBracket], '-', oBracket))

		identMatch = rxIdentifier.findall(identifier)
		numMatch = rxNumber.findall(identifier)
		if len(numMatch) > 0 and numMatch[0] == identifier:
			if readOrWrite == 'READ':
		...
		self.PIF.append(Atom(self.atoms[cBracket], '-', cBracket))
		self.PIF.append(Atom(self.atoms[semiCol], '-', semiCol))

	
	def parseExpression(self, line, hasEnding = True):
		operators = rxOperator.findall(line)
		tokens = [t.strip() for t in rxOperator.split(line)]
		if hasEnding:
			if ';' not in tokens[-1]:
		...
		crtOperator = 0
		for t in tokens:
			identMatch = rxIdentifier.findall(t)
			numMatch = rxNumber.findall(t)
			if len(identMatch) > 0 and identMatch[0] == t:
				if len(t) > 255:
					print 'Error at line %d. Identifier length exceeds 255 (%d)' % (self.lineCount, len(t))
					sys.exit(0)
				# print 'matchuit ', t
				...
				if nFind is None:
					...
					self.symTable.addNode(n)
				self.PIF.append(Atom(self.atoms['ID'], n.code, t))
			elif len(numMatch) > 0 and numMatch[0] == t:
				n = CustomNode(t, self.symTableAddress)
				nFind = self.symTable.findNode(n)
				if nFind is None:
					self.symTable.addNode(n)
	...
	def parseAssign(self, line):
		...
		if not rxIdentifier.match(left):
			self.errSyntax(left)
		if len(left) > 255:
